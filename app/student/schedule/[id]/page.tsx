...
import { getPublishedTimetableForStudent } from "@/actions/timetables";
...
export default async function StudentScheduleDetailPage({ params }: PageProps) {
  ...
  // Get the timetable with assignments filtered for this student group
  const { timetable, error } = await getPublishedTimetableForStudent(
    timetableId,
    studentGroup.id
  );
  ...
  const uniqueInstructors = new Set(
    timetable.assignments.map((a) => a.instructor.name)
  ).size;
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-start mb-2">
          ...
          <SimpleExportButton
            ...
            filters={{ groupId: studentGroup.id }}
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="text-sm text-gray-600 mb-1">Instructors</div>
          <div className="text-2xl font-bold text-gray-900">
            {uniqueInstructors}
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="text-sm text-gray-600 mb-1">Class Hours</div>
          <div className="text-2xl font-bold text-gray-900">
      ...
      {timetable.assignments.length > 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          <p className="text-gray-500 text-lg">
            No classes scheduled for your group in this timetable.
          </p>
        </div>
      )}
              ...
              {timetable.assignments
                ...
                .map((assignment) => (
                  <tr key={assignment.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">{assignment.instructor.name}</td>
                    <td className="py-3 px-4">
